...
import { womens_style_cat, womens_top_cat } from '../../all data/womens'

const Women = () => {
  console.log(womens_style_cat)
  return (
    <div>
      ...
      >
        <p className=" inline-block font-bold text-sm md:text-2xl lg:text-6xl lg:leading-[3.5rem] lg:py-5 sm:py-3 py-1 px-4 rounded-md text-center relative  ">
         Balletcore
        </p>

        <Link to="/products/womens"><div className="bg-slate-400 font-bold text-sm hover:text-slate-100 hover:bg-black duration-300  px-10 lg:py-2 relative top-[1rem] md:top-2 sm:top-5">
        SHOP THE TREND
      </div></Link>
      </div>

      <div className="grid items-center justify-center px-5 m-auto text-center sm:grid-cols-4 gap-x-5">
        {womens_top_cat.map(({ src, id, title, desc }) => {
          return (
      ...
      >
       ...
       <Link to="/products/womens">  <div className="px-10 lg:py-[6px] lg:mt-5 sm:mt-2 lg:text-xl font-bold duration-300 bg-[#ff5400] hover:text-[#ff5400] hover:bg-slate-100 ">
       SHOP NOW
     </div></Link>
      </div>

      <div className="flex flex-col items-center justify-center text-center sm:justify-around gap-y-5 sm:flex-row">
        {womens_style_cat.map(({ src, title, desc, id }) => {
          return (
